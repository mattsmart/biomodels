...
def run_mc_sim(lattice, num_lattice_steps, data_dict, io_dict, simsetup, exosome_string=EXOSTRING, field_remove_ratio=0.0,
    ...
    def meanfield_global_field():
        ...
        neighbours = [[a, b] for a in range(len(lattice[0])) for b in range(len(lattice))]  # TODO ok that cell is neighbour with self as well? should remove diag
...
def mc_sim_wrapper(simsetup, gridsize=GRIDSIZE, num_steps=NUM_LATTICE_STEPS, buildstring=BUILDSTRING, exosome_string=EXOSTRING,
    ...
    lattice, data_dict, io_dict = \
        run_mc_sim(lattice, num_steps, data_dict, io_dict, simsetup, exosome_string=exosome_string, field_remove_ratio=field_remove_ratio,
...
if __name__ == '__main__':
    ...
    random_mem = False
    random_W = True
    simsetup = singlecell_simsetup(unfolding=True, random_mem=random_mem, random_W=random_W, curated=curated,
                                   housekeeping=0)
