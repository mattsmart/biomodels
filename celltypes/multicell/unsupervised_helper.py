...
def plotly_express_embedding(data_subdict, color_by_index=False, fmod='', show=False):
    ...
    for key, algodict in data_subdict['algos'].items():
        ...
        fig.write_image(dirpath + os.sep + "%s_plotly_%s%s.png" % (algo, label, fmod))
        if show:
            fig.show()
    return
...
if __name__ == '__main__':
    ...
    for n_components in [2, 3]:
        ...
        if add_control_data:
            ...
            control_fpath = control_folder + os.sep + 'dimreduce' + os.sep + 'dimreduce%s.z' % fmod
        ...
        if vis_all:
            for idx in range(0, len(manyruns_dirnames)):
                plotly_express_embedding(datasets[idx], fmod=fmod, show=False)
