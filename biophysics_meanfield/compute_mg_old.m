...
pts_mg = length(n2_array) % originally 40
pts2 = 40;
ptsp = 12;
...
dA = 0; %3.0;              % Change in total lattice area in nm^2 per peptide bound (known to be approx 200-300 Angstrom^2 per peptide binding)
H = 0; %-12;               % Hydrophobic energy gain in units of kB T per peptide bound
...
function mu1b = mu1b(i, N1)  % condp Verified
    mu1b = log(4/3 * pi * (0.34)^3*(0.6022*n1eff_mg)) - 0.5*lb*(kappa(i)/(1+0.36*kappa(i)) + (Delta(i)-1)/d1);
end

function mu2b = mu2b(i, N2)  % condp Verified
    mu2b = log(4/3 * pi * (0.43)^3*(0.6022*n2eff(i))) - 2*lb*(kappa(i)/(1+0.43*kappa(i)) + (Delta(i)-1)/d2);
end
...
function mu1c = mu1c(i, a, sigma1, d11)
    kap = kappa(i);
    mu1c = Delta(i) * lb * (-2*pi/kap*(1/a^2-sigma1) + (1/a^2-sigma1) * M1(Q, kap, a) - 1/d1) ...  %************POSSIBLE ERROR************ for mg code, he had M1(kap, a)
...
function mupccc = mupccc(i, a, N1, N2, Np)  % condp Verified
    ...
    % mupccc = delt * lb * (-2*Q*(N2+0.5)*SumC(10,kap,a) + Q*2*pi / kap * (N1 + 2*N2 + Q*Np)/a^2 ...
    %     - Q * (-1*m1+(N1+2*N2+1)*(m1 + mp)/2+Q*Np*mp ... 
    %     - 0.5*(1-1/delt)*(mp-m1))/a^2 ...
    %     this lines bad ^
    %     - Q/dp)+log(Np*(1-Q*Np)^(Q-1)/(0.5-N1-N2-Q*Np)^Q)-(25/4/pi+1-log(Q))+1/(1-Q*Np)+(25/4/pi)/(1-Q*Np)^2 ...
    %     + H + (kA*dA^2/2)*Np/(lattc)^2; %this term is the hydrophobic + mech energy
    % Corrected Mg Version (August 20, 2014)
    mupccc = delt * lb * (-2*Q*(N2+0.5)*SumC(10,kap,a) + Q*2*pi / kap * (N1 + 2*N2 + Q*Np)/a^2 ...
        - Q * (-1*m1+(N1+2*N2+1)*(m1 + mp)/2+Q*Np*mp ... 
        - 0.5*((mp - m1)-1/delt*(a^2/lattc^2)*(Mp(Q, kap, lattc) - M1(Q, kap, lattc))))/a^2 ...
        - Q/dp)+log(Np*(1-Q*Np)^(Q-1)/(0.5-N1-N2-Q*Np)^Q)-(25/4/pi+1-log(Q))+1/(1-Q*Np)+(25/4/pi)/(1-Q*Np)^2 ...
...
%MG CODE
%-0.5*(1-1/delti)(mp - m1))/a^2
%NA ORIGANAL
%-0.5*((mp - m1)-1/delti*(a^2/lattc^2)*(mp_lattc - m1_lattc)))/a^2
...
function condp = condp(i,aa)  % Translation Verified
    ...
    % Mg script guesses (original)
    %x0 = [0.6-3*4.5*n2_array(i)^0.5;
    %     -0.5+3*3*n2_array(i)^0.5;
    %     0.05 - n2_array(i)^0.5];
    % Mg script guesses remade like Na: august 20, 2014
    x0 = [n1^0.5; 
         -0.01-n1^0.5;
         0.01];
    ob = fsolve(@nle,x0);
...
% Plot #1 (Mg modified)
% Plots the fractional site occupancy based on [Mg] for N1, N2, QNp
%{
{Table[{n2[[i]],\[Sigma]pr1[i,lattc]},{i,1,pts2}] , 
Table[{n2[[i]],\[Sigma]pr2[i,lattc]},{i,1,pts2}], 
Table[{n2[[i]],Q*\[Sigma]prp[i,lattc]},{i,1,pts2}]} , 
PlotRange->{{0,1.01*n2[[pts2]]},{-0.1,1}}, 
PlotLegends->{"\!\(\*SubscriptBox[\(N\), \(1\)]\)", "\!\(\*SubscriptBox[\(N\), \(2\)]\)", "\!\(\*SubscriptBox[\(QN\), \(p\)]\)"},
AxesLabel->{"[\!\(\*SuperscriptBox[\(Mg\), \(2+\)]\)](M)","Occupancy"}, PlotLabel-> "Site Occupancy vs [\!\(\*SuperscriptBox[\(Mg\), \(2+\)]\)], \!\(\*SuperscriptBox[\(Na\), \(+\)]\)=0.1M, [AMP]=1\[Micro]M, Revised"]
%}
function plot_frac_site_Mg = plot_frac_site_Mg()
    xlist = n2_array;
    ylist_1 = zeros(1,len);
    ylist_2 = zeros(1,len);
    ylist_p = zeros(1,len);
    ...
    xmax = 1.01*n2_array(len);
    axis([0,xmax,-0.01,1.01]) %xmin xmax ymin ymax
    title(['Fractional Site Occupancy vs [Mg^{2+}]; [Na^{+}] =  ', num2str(n1), ' M, [AMP] = ', num2str(np), ' M'])
    xlabel('[Mg^{2+}] (M)')
    ylabel('Fractional Site Occupancy')
    legend('N_{1} / N_{0} (Na^{+})','N_{2} / N_{0} (Mg^{2+})','Q*N_{p} / N_{0} (AMP)','Location','northeast')
    saveas(h, 'mg_frac_site_MH.jpg')
end

% Plot #2 (Mg modified)
% Plots the fractional CHARGE occupancy based on [Mg] for N1, N2, QNp
%
function plot_frac_charge_Mg = plot_frac_charge_Mg()
    xlist = n2_array;
    ylist_1 = zeros(1,len);
    ylist_2 = zeros(1,len);
    ylist_p = zeros(1,len);
    ...
    xmax = 1.01*n2_array(len);
    axis([0,xmax,-0.01,1.01]) %xmin xmax ymin ymax
    title(['Fractional Charge Occupancy vs [Mg^{2+}]; [Na^{+}] =  ', num2str(n1), ' M, [AMP] = ', num2str(np), ' M'])
    xlabel('[Mg^{2+}] (M)')
    ylabel('Fractional Charge Occupancy')
    legend('N_{1} / N_{0} (Na^{+})','2*N_{2} / N_{0} (Mg^{2+})','Q*N_{p} / N_{0} (AMP)','Location','northeast')
    saveas(h, 'mg_frac_charge_MH.jpg')
end

% Plot Free Energy
% Free energy to understand tension
function plot_freep_Mg = plot_freep_Mg()    
    aa = lattc;
    xlist = n2_array;
    len = length(xlist);
    ...
    title(['Free Energy vs [Mg^{2+}]; [Na^{+}] =  ', num2str(1000*n1),' mM, [AMP] = ', num2str(1000*np), ' mM'])
    xlabel('[Mg^{2+}] (M)')
    ylabel('Free Energy (k_{B} T)')
    saveas(h, 'mg_freep_MH.jpg')    
end
...
function plot_tensionp_Mg = plot_tensionp_Mg()    
    aa = lattc;
    xlist = n2_array;
    ...
    axis([0,n2_array(len),-0.6,1.0])
    title(['\Delta \Pi vs [Mg^{2+}]; [Na^{+}] =  ', num2str(n1),' M, [AMP] = ', num2str(np), ' M'])
    xlabel('[Mg^{2+}] (M)')
    ylabel('\Delta \Pi (k_{B} T / nm^2)')
    saveas(h, 'mg_tensionp_MH.jpg')
end
...
function get_custom_data = get_custom_data()
    ...
    M = real([n2_array', ylist_1', ylist_2', ylist_22', ylist_p', ylist_free_p', ylist_tensionp', ylist_tensionmech'])
    filename = sprintf('data\\mg_old_data_na_%d_mM_amp_%1.1f_muM_k_%d.txt', 1000*n1, 1e6*np, k0);
    fid = fopen(filename, 'w');
    col_id = sprintf('_na_%d_amp_%1.1f_k_%d_old', 1000*n1, 1e6*np, k0);
    col_header = sprintf('Mg,n1%s,n2%s,2_n2%s,Q_np%s,Free%s,Ten%s,TenMech%s\n', col_id, col_id, col_id, col_id, col_id, col_id, col_id);
...
function get_data_no_AMP = get_data_no_AMP()
    ...
    ylist_1 = zeros(1,len);
    ylist_2 = zeros(1,len);
    ylist_free_2 = zeros(1,len);
    ylist_tension2 = zeros(1,len);
    for i = 1:len
        cond2_tmp = cond2(i,lattc, d1, d2);
        ylist_1(i) = cond2_tmp(1); % from sigmapr1(i,lattc)
        ylist_2(i) = 0.5 + cond2_tmp(2); % from sigmapr2(i,lattc)
        ylist_free_2(i) = FreeEnergy2(i,lattc);
        ylist_tension2(i) = Tension2(i,lattc);
    end
    ylist_22 = 2*ylist_2; % reps frac charge occ for Mg
    M = real([n2_array', ylist_1', ylist_2', ylist_22', ylist_free_2', ylist_tension2'])
    filename = sprintf('data\\mg_custom_na_%d_mM_amp_ZERO_muM.txt', 1000*n1);
    fid = fopen(filename, 'w');
    fprintf(fid, '[Mg],N_1 / N_0,N_2 / N_0,2*N_2 / N_0,Free Energy,Tension\n');
    fclose(fid)
    dlmwrite(filename,M,'-append','precision',4)
    get_data_no_AMP = M;
end
...
function get_tension_data = get_tension_data()
    ...
    M = real([n2_array', ylist_tensionmech', ylist_tensionelec'])
    get_tension_data = M;
end
