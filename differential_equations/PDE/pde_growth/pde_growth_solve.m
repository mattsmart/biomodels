...
% geometry
hmax = 0.1;  % options 0.1, 0.05, 0.02 -- lower is more dense
model = createpde(N);
...
u0_bacteria(ix_bacteria) = b0 * ones(size(ix_bacteria));
%u0_nutrients = n0 * ones(np,1);
u0_nutrients = zeros(np,1);
ix_nutrients = find(sqrt(p(1,:).^2 + p(2,:).^2) < 0.8);  % circle with value 1
u0_nutrients(ix_nutrients) = n0 * ones(size(ix_nutrients));
...
c = @pde_growth_coeff_c;    % interpreted as N^2 x dim^2 x 1
